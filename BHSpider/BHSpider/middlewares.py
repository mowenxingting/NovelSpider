...
class BhspiderSpiderMiddleware(object):
...
class BhspiderDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class HttpbinProxyMiddleware(object):
    ...
    #     pass
    def __init__(self,crawler):
        self.proxy_addrs = crawler.settings.get("PROXY_ADDRS")

    def process_exception(self, request, response, spider):
        self.logger.debug('Try Exception time')
        self.logger.debug('Try second time')
        proxy_addr_get=self.proxy_addrs[random.randint(0,len(self.proxy_addrs)-1)]+'/get'
        proxy_addr = requests.get(proxy_addr_get).text
